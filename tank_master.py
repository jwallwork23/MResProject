...
if compare != 't':
    ...
    # Initialise time, counters and output:
    t = 0.0
    dumpn = 0
    i = 0
    q_file.write(u, eta, time = t)

    # Initialise arrays for storage (pre-allocating dimension for speed):
    ...
    while t < T - 0.5*dt:
        t += dt
        if waves == 'y':
            bcvals.assign(wave_machine(t, A, p, in_flux))                       # Update boundary conditions
        q_solv.solve()
        q_.assign(q)
        dumpn += 1
        if dumpn == ndump:
...
if compare != 's':
    ...
    if compare == 'b':
        ...
        # Run the model:
        if waves == 'y':
            solver_obj.iterate(update_forcings = update_forcings, export_func = plot_difference)
        else:
            solver_obj.iterate(export_func = plot_difference)
