...
def my_transfer_solution(adaptor, *fields, **kwargs):
    """
    ...
    for f in fields:
        ...
        if f.ufl_element().family() == 'Lagrange' and f.ufl_element().degree() == 1:
            try:
                f_new.dat.data[:] = f.at(adaptor.adapted_mesh.coordinates.dat.data)
                ...
                for v in range(vStart, vEnd):
                    ...
                    try:
                        ...
                        val = 0.
                        notInDomain.append([v, INF, -1])  # (Vertex, distance, edge) tuple
                    finally:
            ...
            while len(notInDomain) > 0:
                ...
                for tup in notInDomain:
                    ...
                    try:
                        ...
                        print '####  Vertex still not in domain: %f %f' % (newCrd[0], newCrd[1])
                        val = 0.
                    finally:
                        f_new.dat.data[offnew] = val
                        notInDomain.remove(tup)
                if eps >= 100:
